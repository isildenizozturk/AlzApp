...
const UserFormScreen = (props) => {
    ...
    const showQuizToast = () => {
        ToastAndroid.show("Lütfen önce bilgilerinizi giriniz", ToastAndroid.SHORT);
    };
                ...
                <ScrollView>
                    <View style={styles.form}>
                        <View style={styles.formControl}>
                            ...
                            {isfirstnamevalid === 2 ? <Text style={styles.validateMessage}>Lütfen isminizi giriniz.</Text> : <Text></Text>}
                        </View>
                        <View style={styles.formControl}>
                            <Text style={styles.label}>Soyisim</Text>
                            <TextInput style={styles.input} value={lastname} onChangeText={text => { setLastName(text); setIsLastNameValid(0) }} />
                            {islastnamevalid === 2 ? <Text style={styles.validateMessage}>Lütfen soyisminizi giriniz.</Text> : <Text></Text>}
                        </View>

                        <View style={styles.formControl}>
                            <Text style={styles.label}>Yaş</Text>
                            <TextInput style={styles.input} value={age} onChangeText={text => { setAge(text); setIsAgeValid(0) }} keyboardType="numeric" />
                            {isagevalid === 2 ? <Text style={styles.validateMessage}>Lütfen geçerli bir yaş giriniz.</Text> : (parseInt(age) >=18 && parseInt(age) <= 100 && age.trim().length > 0 ? null : <Text>18 ile 100 arası bir değer giriniz.</Text>)}
                        </View>
                    </View>
